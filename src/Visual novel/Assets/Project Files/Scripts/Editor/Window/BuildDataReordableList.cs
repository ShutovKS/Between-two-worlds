...
namespace Editor.Window
{
    public class BuildDataReordableList
    {
        ...
        {
            var reorderableList =
                new UnityEditorInternal.ReorderableList(configsList, typeof(BuildData), true, false, true, true);
            ...
            reorderableList.drawElementCallback = (position, index, isActive, isFocused) =>
            {
                ...
                var buildOptionsWidth =
                    position.width - space * 6 - enabledWidth - buildTargetWidth - buildTargetGroupWidth;
            ...
            reorderableList.onAddDropdownCallback = (buttonRect, list) =>
            {
                ...
                {
                    var label = /*"WebGL/"+ */ BuildManager.Core.BuildManager.ConvertBuildTargetToString(config.target);
                    menu.AddItem(new GUIContent(label), false, menuItemHandler, config);
                }
